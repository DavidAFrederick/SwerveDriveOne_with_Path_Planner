...
#    swerve.requests.ApplyRobotSpeeds()
    ...
    def test1 (self):
        # Define the speeds
        forward_speed = 1.0  # 1 meter per second forward
        left_speed = 0.0     # 0 meters per second to the left
        rotation_speed = 0.0 # 0 radians per second rotation

        # Create a request object with the desired speeds
        request = self.SwerveRequest.ApplyRobotSpeeds(
            forward_speed=forward_speed,
            left_speed=left_speed,
            rotation_rate=rotation_speed
        )

        # Apply the request to the drive
    ...
    #         .with_velocity_x( 0.5 )
    #         .with_velocity_y( 0.5 )
    #         .with_rotational_rate( 0.5 )
    #     )
        
    #     # Apply the drive request to the swerve drivetrain
    #     self.swerve_drivetrain.apply_request(lambda: drive_request)
